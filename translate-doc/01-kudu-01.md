...
base data 在 DiskRowsets中按照列组织的性质存在，每一列是以一个单独的连续数据块分别写入磁盘。列本身被分割为更小的页以便于支持随机读。而且一个嵌入式B树索引支持基于Rowset范围内的偏移量的高效查找。列页可以用各种编码技术编码，比如：字段编码，位shuffle，front coding ，并且可以选择性的使用二进制压缩模式，比如$LZ4,gzip,bzip2$。这些编码和压缩选项可能被用户根据字段属性锁指定。
