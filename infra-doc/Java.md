...
==class 本身是一种数据类型，这个角度就很犀利==

本类中的方法可以直接访问本类中的成员变量。**特殊**：static成员访问非static成员，编译不通过。

### 1.1-内部类

当一个事物的内部，还有一个部分需要一个完整的结构进行描述，而这个内部的完整的结构又只为外部事物提供服务，那么整个内部的完整结构最好使用内部类。而且**声明为内部类可以直接使用/访问外部类的所有的成员，包括私有的**。JRE核心类库集合的迭代器大量使用内部类。
...
* 在运行期间，动态的为某个对象的某个属性赋值，获取某对象的属性值
* 在运行期间，动态的调用某个对象的某个方法

### 2.2-反射的根源（java.lang.Class）
